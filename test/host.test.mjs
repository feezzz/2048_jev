...
/* -------------------------------------------------------------------------
   3.5 结束遮罩上的「再来一局」必须真的能开新局
   ------------------------------------------------------------------------- */
{
  ok(!$('overlay').hidden, '这一局结束后遮罩应当还亮着')

  const peakBoard = tileCount()
  ok(peakBoard > 2, `结束后棋盘应当是满的，实际 ${peakBoard} 个方块`)

  $('overlay-btn').click()
  await sleep(400)

  ok(score() === 0, `点「再来一局」后分数应归零，实际 ${score()}`)
  ok(tileCount() === 2, `点「再来一局」后应只剩 2 个方块，实际 ${tileCount()}`)
  ok($('overlay').hidden, '点「再来一局」后遮罩应当收起')
  ok($('ai-toggle').checked === false, '重开后托管仍应保持关闭')

  console.log('  ✓ 点「再来一局」：成功开新局，遮罩收起')
}
